//! Mint release fixtures, so the real `updaterd` and `robotctl` can be driven by hand.
//!
...
//! nothing in the tree hands you is a *release to feed them*: `xtask package`
...
//!
//! ```text
//!   <root>/r/<version>/       each release, minted but not offered
...
//!
//! Ask for every version you need up front. The tool refuses to add to a populated tree so
//! each run remains deterministic and callers cannot accidentally mix fixture state.
...
#[derive(Parser)]
#[command(about = "Mint releases for driving updaterd and robotctl by hand")]
struct Cli {
    ...
    ///
    /// `tamper` corrupts the artifact after hashing (must be refused), `hook` embeds a
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Refuse rather than add to an existing tree: mixing runs would leave stale references
    // in `published/` and make a test's resolved `latest` depend on prior local state.
    ...
    for dir in ["published", "opt/daemon", "var"] {
